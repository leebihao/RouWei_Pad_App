...
public class TcpTransport extends ServiceBase implements Transport {
    ...
    class CANCELING extends SocketState {
        ...
        void onCanceled() {
            trace("CANCELING.onCanceled" + remaining);
            remaining--;
            if( remaining!=0 ) {
                return;
            }
...
//            }
    ...
    public void drainInbound() {
        ...
        if (BuildConfig.PUSH_DEVICE) {
            if (TextUtils.isEmpty(Configuration.getConnectId())) {
                return;
            }
        } else {
        ...
        try {
            ...
            LogUtil.d("=====initial=" + initial);
            LogUtil.d("=====codec.getReadCounter()=" + codec.getReadCounter());
            LogUtil.d("=====codec.getReadBufferSize()=" + codec.getReadBufferSize());
            LogUtil.d("=====codec.getReadBufferSize()<<2=" + (codec.getReadBufferSize()<<2));
            ...
            while( codec.getReadCounter()-initial < codec.getReadBufferSize()<<2 ) {
                ...
                TcpTransportWakeLock.acquireWakeLock(context);
                Object command = codec.read();
                trace("=========receive the message");
                if ( command!=null ) {
                    try {
    ...
    public void disconnect() {
        if (isConnected()) {
            ((CONNECTED)socketState).onCanceled();
        }
    }
